...
Create fresh PNG artwork with `references/056-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/056-source.md` completely immediately before building every generation request.
...
1. 上下对照（原图在上，056 设计在下）
2. 左右对照（原图在左，056 设计在右）
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/056-source.md,
...
- `references/xxd-panel-056-prompt.zh-CN.md`
- `references/xxd-panel-056-prompt.en.md`

These files document the same minimal adapter. They never replace `references/056-source.md`.
