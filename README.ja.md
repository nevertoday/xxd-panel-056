...
# 🦁 XXD Panel 056｜余白の記憶画

### 核心イメージを一つだけ残し、巨大な余白・止まる筆触・暖冷の跳色で描き直す
...
<table>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091730850723586307"><img src="./assets/examples/sample-01.jpg" alt="XXD Panel 056 作例 1"></a></td>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091730850723586307"><img src="./assets/examples/sample-02.jpg" alt="XXD Panel 056 作例 2"></a></td>
  </tr>
  <tr>
    <td width="50%"><a href="https://x.com/xiaoxiaodong01/status/2091730850723586307"><img src="./assets/examples/sample-03.jpg" alt="XXD Panel 056 作例 3"></a></td>
...
上の 16:9 作例と同じ4点の独立素材を、完全な 3:4 上下構成として再生成しました。上部は現実の写真を保ち、下部は Panel 056 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 056 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 056 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 056 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 056 追加上下作例 4"></td>
  </tr>
</table>

> [小小東（@xiaoxiaodong01）](https://x.com/xiaoxiaodong01/status/2091730850723586307) · 2026年8月24日<br>
> GPT2 × 暖色 × アーティストの色盤 × 美学プロンプト × VOL.056

<p align="center"><a href="https://x.com/xiaoxiaodong01/status/2091730850723586307">元の投稿と完全なプロンプトを見る →</a></p>

これらの作例は 056 の美的成果だけを示します。作例の被写体、構図、配色、文字、旧キャンバス比率が、新しいタスクの参照や既定値になることはありません。
...
**Panel 056** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**核心イメージを一つだけ残し、巨大な余白・止まる筆触・暖冷の跳色で描き直す**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 056 でこの画像を処理して、まず最適な構図とサイズを提案して。
...
## すぐに判断：XXD Panel 056 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 核心イメージを一つだけ残し、巨大な余白・止まる筆触・暖冷の跳色で描き直す |
| **ひと目で分かる特徴** | 核心イメージ · 巨大な余白 · 暖冷の跳色 · 稚拙な手描き · 視覚的比喩 |
...
写真が本来持つ核心イメージ、視覚的比喩、物同士の意味ある距離を探し、一つの主体とごく少数の物語関係だけを残します。全ての物を描き替えたり背景を埋めたりせず、空白に空気、時間、感情を担わせます。

```text
同一性と重要な姿勢を固定 → 核心イメージ／比喩を判断 → 無関係な環境と物を削除 → 一つの主体とごく少数の関係を残す → 白に近い巨大な余白で距離を作る → 不透明な面、止まる筆触、下地の露出、局所的な色ずれで再構成 → 意味のある最小限の文字を統合
```

## 完成作品で見分けやすいスタイル特性

- 明確に識別できる主体一つと、物語に本当に関わる物や空間関係だけを残します。
- 元写真の色温度に応じて変わる白に近い背景と、感情を担う巨大な負の空間。
- 不透明なガッシュ、アクリル、パステル、乾いた油彩が、停止、修正、下地、少し不器用な手を残します。
- クリーム黄、杏、桃、珊瑚、オールドローズと、空色、ミント、灰緑がゆるく暖冷対比し、局所的な色ずれも許します。
- 主体は偏心、浮遊、低重心、部分的な裁ち落としが可能で、文字は機械的な情報ではなく絵の一部として余白に入ります。

美的制約と拒否項目は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあり、Skill とランタイムアダプターは今回の出力変数だけを扱います。 [Skill](SKILL.md) · [英語ランタイムアダプター](references/xxd-panel-056-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 056 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
# フォルダー全体へ Panel 056 を適用し、共通設定は一度だけ解決
/xxd-panel-056 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-056`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-056-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-056-prompt.zh-CN.md)
